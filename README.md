...
sequenceDiagram
    ...
    activate MoveGen
    %%MoveGen ->> MoveGen: pseudoLegalMoves()
    %%loop generate moves per piece
    %%    MoveGen ->> Position: read board squares
    %%    MoveGen ->> MoveObj: new Move(...) 
    %%end
    %%MoveGen ->> Rule: for each pseudo-move: makeMove -> check inCheck
    %%activate Rule
    %%Rule ->> Position: Position.makeMove(m)  // simulate
    %%Rule ->> Rule: isSquareAttacked(kingSquare, attacker)
    %%Rule -->> MoveGen: legal? (true/false)
    %%deactivate Rule
